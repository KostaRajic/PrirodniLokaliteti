import jagodina1 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina1.jpg";
import jagodina2 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina2.jpg";
import jagodina3 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina3.jpg";
import jagodina4 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina4.jpg";
import jagodina5 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina5.jpg";
import jagodina6 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina6.jpg";
import jagodina7 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina7.jpg";
import jagodina8 from "../../../../../assets/images/SumadijaIZapadnaSrbija/PomoravskiOkrug/Jagodina/jagodina8.jpg";

import { FaArrowLeft } from "react-icons/fa";
import { useNavigate } from "react-router-dom";
import { ScrollToTop } from "../../../../../ScrollToTop";

export const Jagodina = () => {
  const navigate = useNavigate();

  return (
    <div className="placeBackground">
      <ScrollToTop />
      <FaArrowLeft
        className="arrowLeft"
        onClick={() => navigate(-1)}

      />
      <header></header>
      <img src={jagodina2} alt="Jagodina" />

      <h2 style={{ color: "white" }}>Jagodina</h2>

      <p style={{ paddingTop: "50px" }}>
        Jagodinska opština prostire se na 470 km², obuhvata 54 naselja i graniči
        se sa osam opština. Na severu se graniči sa opštinom Svilajnac, na
        severoistoku sa opštinom Despotovac, a na severozapadu sa opštinom
        Batajnica. Na istoku i jugoistoku graniči se sa opštinama Paraćin i
        Ćuprija, na zapadu sa gradskom opštinom Pivara grada Kragujevca,
        jugozapadno sa opštinom Rekovac, a na jugu sa opštinom Varvarin.
        Jagodina se nalazi na 43° 59' severne geografske širine i 24° 14'
        istočne geografske dužine, na prosečnoj nadmorskoj visini od 116 m, a
        najviša tačka je Đurđev brdo sa 213 m.
      </p>

      <img src={jagodina1} alt="Jagodina Djurdjevo Brdo Vodopad" />

      <p>
        Dana 22. septembra 1946. godine, odlukom opštine, Jagodina je dobila ime
        Svetozarevo u čast socijaliste Svetozara Markovića. Romantičari i
        sanjari pominju krčmaričicu Jagodu, sa dvorištima i hanom na Crvenom
        bregu iznad groblja, u čijem je podnožju raslo selo, nazvano po njoj –
        Jagodina. Prema rečima njenih poštovalaca, krčmaričica Jagoda je po danu
        ugostila putnike, a noću, pod mesečinom, sa 40 razbojnika napadala i
        pljačkala bogate karavane na Carigradskom drumu. To bi trajalo zauvek da
        legenda ima svoj kraj, a priča kaže da je u jednoj pljački srela mladića
        čija lepota je naterala njenu razbojničku ruku da zadrhti. Njeni
        iznevereni prijatelji su je pustili da ga odvede u svoje dvore, ali
        izdajstvo joj nikada nije bilo oprošteno. Jedne noći nestao je u plamenu
        Jagodin han, a nad njegovim zgarištem, u mračnim noćima, godinama se
        čulo njeno jezivo kikotanje. Stariji se sećaju da je kraj groblja
        postojao komad neobrađene zemlje, koji se do skoro zvao opštinska
        utrina, a nekada davno – Jagodina livada.
      </p>

      <img src={jagodina3} alt="Park na Djurdjevom brdu" />

      <p>
        Šetalište Potok "Đurđevo brdo" je prelep i pažljivo uređen park sa
        klupama, cvećem, potokom i izvornom česmom, savršen za uživanje,
        opuštanje i šetnje. 2015. godine, park je obogatio niz fontana i
        veštački vodopad, koji dodatno upotpunjuju njegovu lepotu. Prirodna,
        mirna atmosfera parka pruža idealno okruženje za potpuni odmor, a u
        njegovom središtu se nalazi i letnja pozornica na kojoj se leti
        održavaju kulturne i umetničke manifestacije. U neposrednoj blizini
        šetališta smešteni su i Zoološki vrt, Muzej voštanih figura, Aqua park i
        šoping centar VIVO.
      </p>

      <img src={jagodina4} alt="Akva Park Jagodina" />

      <p>
        Akva park u Jagodini, prvi takav park u Srbiji, otvoren je 2007. godine.
        Prostire se na 50.000 kvadratnih metara i nudi posetiocima 7 bazena i 9
        tobogana. Ovo je savršen način da se pobegne od gradske vreve tokom
        toplih letnjih meseci i uživa u celodnevnoj zabavi. Park obuhvata
        olimpijski bazen, rekreativni bazen i bazene za neplivače, a posebno je
        prilagođen deci, sa plitkim bazenima i manjim toboganima. Glavne
        atrakcije parka uključuju: Gusarski brod – idealan za decu, sa plitkim
        bazenom i kratkim, jednostavnim toboganima. Tu su i mlaznjaci koji
        izbacuju vodu, kao i kofa koja neprestano pada. „Rafting slide“ –
        uživajte u spuštanju sa gumama zajedno sa decom, partnerom ili
        prijateljima na ovom širokom toboganu. „Wide slide“ – tobogan bez
        krivina, koji se pravo spušta u vodu, savršen za one koji ne vole visinu
        i brzinu. Tunel „Body slide“ – uzbudljivo iskustvo zatvorenog tobogana
        koji lišava čula vida, pa ostala čula postaju intenzivnija. Super „Multi
        slide“ – ovo je tobogan sa tri staze, pa možete da se spuštate zajedno
        sa prijateljima. Spiral „Body slide“ – otvoreni tobogan sa zavojima koji
        vas vodi do dna. „Multi slide“ – tobogan sa dve staze i oštrim spustom,
        koji nije za slabijeg srca. Osim vodenih atrakcija, Akva park nudi i
        četiri kafića, restoran i bar, gde možete uživati u obroku ili
        osveženju. U baru čak možete naručiti piće dok ste u vodi.
      </p>

      <img src={jagodina5} alt="Zavičajni muzej u Jagodini" />

      <p>
        Zavičajni muzej u Jagodini postao je značajna kulturna institucija u
        ovom gradu, ali i u celom Pomoravlju, koja prikuplja desetine hiljada
        vrednih predmeta iz oblasti prirodnih nauka, arheologije, istorije,
        etnologije i umetnosti. Zbog ograničenog prostora, trenutno je izložen
        samo manji deo ove bogate zbirke. Muzej je konačno dobio adekvatan
        prostor u zgradi bivšeg Sokolskog doma, izgrađenog 1935. godine, koja je
        sama po sebi kulturni spomenik. Ovaj prostor je omogućio postavljanje
        stalne izložbe, pa svaki posetilac ima jedinstvenu priliku da na jednom
        mestu sazna više o vekovnim tokovima života i rada na ovom području. Pre
        nego što je izgrađen Sokolskog dom, na ovom zemljištu se nalazila kuća
        uglednog Jagodinca, Stevana Stevče Mihajlovića, koji je bio jedan od
        učitelja mladog kneza Milana Obrenovića.
      </p>

      <img src={jagodina6} alt="VIVO shopping PARK u Jagodini" />

      <p>
        VIVO shopping PARK prostire se na 25.000 m², dok sam objekat zauzima
        10.000 m². Unutar njega nalazi se 33 maloprodajna lokala, koja su
        popunjena poznatim međunarodnim i domaćim brendovima, kao i 360 parking
        mesta. Celi kompleks je pažljivo dizajniran u jedinstvenu arhitektonsku
        celinu koja se odlikuje funkcionalnošću, lakoćom pristupa, preglednošću,
        kao i usklađenošću sa najsavremenijim svetskim standardima gradnje,
        uključujući kvalitet materijala, energetsku efikasnost, ekološke norme,
        sigurnost i zaštitu od požara. Ponuda u parku obuhvata širok spektar
        proizvoda, uključujući supermarket, odeću, obuću, sportsku opremu, dečju
        garderobu, igračke, nameštaj, elektroniku, kozmetiku, restoran, apoteku,
        knjižaru i mnogo više. Naša ponuda karakteriše visok kvalitet, moderne
        linije i pristupačne cene.
      </p>

      <img src={jagodina7} alt="Muzej naivne i marginalne umetnosti u Jagodini" />

      <p>
        Muzej naivne i marginalne umetnosti u Jagodini predstavlja kulturnu
        instituciju od nacionalnog značaja, koja čuva riznicu od oko tri i po
        hiljade dela više od četiristo umetnika iz četrdesetak zemalja, svih
        generacija, od početka 20. veka do danas. Umetnička zbirka muzeja
        (slike, skulpture, grafike, crteži) sastoji se od otkupljenih dela,
        poklona i naknada prilikom organizovanja izložbi, kolonija i Bijenala
        naivne i marginalne umetnosti. Muzej je smešten u impozantnoj zgradi,
        koja je najlepši primer secesije s početka 20. veka u Jagodini, i nalazi
        se u srcu gradskog parka, pored glavnog šetališta.
      </p>

      <img src={jagodina8} alt="Zoološki vrt u Jagodini" />

      <p>
        Zoološki vrt u Jagodini jedan je od četiri zoološka vrta u Srbiji i
        zvanično je otvoren 10. jula 2006. godine. Prostire se na površini od
        2,5 hektara, na zemljištu koje je prethodno bilo neobrađeno. U njemu se
        nalazi oko 250 vrsta životinja, sa više od 320 jedinki. Posetioci mogu
        koristiti usluge vodiča koji ih vode kroz kompleks i pružaju detaljna
        objašnjenja o životinjama. Zoo-vrt je otvoren 365 dana u godini, bez
        obzira na verske ili državne praznike. Zoološki vrt u Jagodini je prvi
        takav objekat izgrađen južno od Beograda, a njegovim otvaranjem,
        Jagodina je postala jedan od važnih turističkih centara u Srbiji.
        Svečano ga je otvorio Vuk Bojović, bivši direktor Beo zoo vrta i počasni
        građanin Jagodine. Zoo-park se nalazi u Jagodini, u Pomoravlju, unutar
        izletničkog parka Đurđevo brdo, koji je zaštićen kao spomenik prirode.
      </p>
    </div>
  );
};
